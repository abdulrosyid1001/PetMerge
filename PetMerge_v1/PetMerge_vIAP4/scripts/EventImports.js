...
var gameId = "petmerge_test";

if(platform_ad !== "Azerion"){
    gameId = "petmerge"+"_"+platform_ad.toLowerCase();
}
else{
    gameId = "petmerge_build"; // id game terakhir di azerion.
}
...
function createAdBanner() {
    ...
    ///OLDXIAOMI BANNER  //enabled or disabled to activated the banner with the some platform
    // ins element ads
    ...
    // banner_container.style.height = "60px";
                ...
                if (!is_loaded_banner_ad) {
                    try {
                        ...
                        await window.gdsdk.showAd("display", { containerId: "bannerDiv" })
                            .then(() => {
                                console.info("showAd(window.gdsdk.AdType.Display) resolved.");
                            })
                            .catch((error) => {
                                console.info("Azerion Banner Error:", error);
                            });
                    } catch (err) {
                        console.info("Azerion Banner Exception:", err);
                    }
                }
            } else {
                console.info("gdsdk not available or showAd is not a function.");
            }
        }
    }
...
function timeFormat(dataTime) {
    const totalSeconds = Math.floor(dataTime / 1000);
    const hours = String(Math.floor(totalSeconds / 3600)).padStart(2, '0');
    const minutes = String(Math.floor((totalSeconds % 3600) / 60)).padStart(2, '0');
    // const seconds = String(totalSeconds % 60).padStart(2, '0');
    var finalTimeFormat = `${hours}h ${minutes}m`;
    return finalTimeFormat;
}

var tempDataLimitedOffer = {};
var tempExpTimeLimitedOffer = [0, 0];
var tempShopData = {};
var tempBannerData = {};
var tempLOData = {};
...
//     try {
//         runtime.callFunction("set_checkpoint_tiles");
//     } catch (err) {
//         console.error("Error panggil function:", err);
//     }
// });
